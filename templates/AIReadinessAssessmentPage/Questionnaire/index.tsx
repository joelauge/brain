...
import { aiReadinessQuestions, ReadinessAssessment, calculateReadinessScore } from "@/mocks/ai-readiness-questions";
...
const Questionnaire = ({ onComplete }: QuestionnaireProps) => {
    ...
    const question = aiReadinessQuestions[currentQuestion];
    const progress = ((currentQuestion + 1) / aiReadinessQuestions.length) * 100;
    ...
    const handleAnswer = (value: any) => {
        ...
        if ((question.type === 'single' || question.type === 'scale') && currentQuestion < aiReadinessQuestions.length - 1) {
    ...
    const handleNext = () => {
        if (currentQuestion < aiReadinessQuestions.length - 1) {
            setCurrentQuestion(currentQuestion + 1);
        } else {
    ...
    const handleEmailSubmit = async () => {
        ...
        // Calculate scores
        const scores = calculateReadinessScore(assessment);
        assessment.overallScore = scores.overallScore;
        assessment.categoryScores = scores.categoryScores;
        assessment.roadmapStage = scores.roadmapStage;
        assessment.roadmapStageName = scores.roadmapStageName;

        // Send to API
        try {
            await fetch('/api/ai-readiness-assessment/capture', {
                method: 'POST',
                headers: {
                    ...
                    <div className="bg-n-8 rounded-2xl border border-n-6 p-8 md:p-12">
                        <div className="mb-8 text-center">
                            ...
                            <p className="body-2 text-n-3">
                                Please provide your contact information to receive your personalized AI readiness assessment results.
                            </p>
                        </div>
            ...
            <div className="container max-w-[70rem]" ref={questionRef}>
                ...
                <div className="mb-15">
                    <div className="flex items-center justify-between mb-4">
                        <Tagline>Question {currentQuestion + 1} of {aiReadinessQuestions.length}</Tagline>
                ...
                <div className="bg-n-8 rounded-2xl border border-n-6 p-8 md:p-12">
                    ...
                    <div className="flex justify-between pt-6 border-t border-n-6">
                        ...
                        <Button
                            ...
                            white
                        >
                            {currentQuestion === aiReadinessQuestions.length - 1 ? 'Complete Assessment' : 'Next'}
                        </Button>
                    </div>
                </div>
            </div>
