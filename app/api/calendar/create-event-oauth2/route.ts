...
export async function POST(request: NextRequest) {
    try {
        ...
        // Skip creating calendar events in development if no OAuth2 credentials
        if (!process.env.GOOGLE_OAUTH2_CREDENTIALS) {
            console.log('📅 [DEV] OAuth2 Calendar Event (not created):', {
            ...
            return NextResponse.json({ 
                success: true, 
                message: 'OAuth2 Calendar event creation skipped in development',
                eventId: 'dev-mock-event'
            });
        }

        // Parse OAuth2 credentials
        const credentials = JSON.parse(process.env.GOOGLE_OAUTH2_CREDENTIALS);
        
        // Initialize Google Calendar API with OAuth2
        const auth = new google.auth.OAuth2(
            credentials.client_id,
            credentials.client_secret,
            credentials.redirect_uris?.[0] || 'http://localhost:3000/auth/google/callback'
        );
        ...
        console.log('📅 OAuth2 Calendar Event Created:', response.data.htmlLink);
        
        return NextResponse.json({ 
            success: true, 
            eventId: response.data.id,
            eventLink: response.data.htmlLink,
            message: 'OAuth2 Calendar event created successfully with attendee invitations'
        });

    } catch (error: any) {
        console.error('Error creating OAuth2 calendar event:', error);
        ...
        if (error.message?.includes('invalid_grant')) {
            return NextResponse.json({ 
                ...
                solution: 'Run the token generator script again to get a new refresh token.'
            }, { status: 401 });
        }
        ...
        if (error.message?.includes('No access, refresh token')) {
            return NextResponse.json({ 
                ...
                details: 'Missing refresh token or access token in credentials.',
                solution: 'Run the token generator script to get proper OAuth2 credentials.'
            }, { status: 401 });
        }
        
        return NextResponse.json({ 
            success: false, 
            error: 'Failed to create OAuth2 calendar event',
