...
export default function TagPage() {
  ...
  const fetchArticles = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/ai-news?tag=${encodeURIComponent(tag)}`);
      ...
      <Section className="pt-12 pb-12">
        <div className="container">
          ...
          <div className="mb-8">
            ...
            <h1 className="h1 text-n-1 mb-4">Tag: {tag}</h1>
          ...
          {!loading && (
            <>
              {paginatedArticles.length === 0 ? (
                <div className="text-center py-12">
                  <p className="body-1 text-n-3">No articles found with this tag.</p>
                </div>
              ) : (
                <>
                      ...
                      >
                        ...
                        {article.tags && (
                          <div className="flex flex-wrap gap-2">
                            {article.tags.split(',').slice(0, 3).map((tagItem, idx) => (
                              <span
                                key={idx}
                                className={`px-2 py-1 rounded text-xs ${
                                  tagItem.trim().toLowerCase() === tag.toLowerCase()
                                    ? 'bg-color-1/30 text-color-1'
                                    : 'bg-n-7 text-n-3'
                                }`}
                              >
                                {tagItem.trim()}
